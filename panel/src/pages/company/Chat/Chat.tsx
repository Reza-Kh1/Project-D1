...
import { fetchApprovedChatCompany, fetchChatCompany } from '../../../services/company/chat'
import { Outlet, useNavigate, useParams } from 'react-router-dom'
export default function ChatCompany() {
  const { id } = useParams()
  const navigate = useNavigate()
  const [isLast, setIsLast] = useState<boolean>(false)
  const [isShow, setIsShow] = useState<boolean>(false)
  const { data } = useQuery<{ data: AllChatType[] }>({
    queryKey: ["AllChatCompany"],
    queryFn: fetchChatCompany,
    staleTime: 1000 * 60 * 60 * 24,
    gcTime: 1000 * 60 * 60 * 24,
  });
  const { data: approvedChat, refetch } = useQuery<{ data: AllChatType[] }>({
    queryKey: ["ApprovedChatCompany"],
    queryFn: fetchApprovedChatCompany,
    staleTime: 1000 * 60 * 60 * 24,
    gcTime: 1000 * 60 * 60 * 24,
    enabled: isLast
  });
  const NavLink = ({ href, text, image }: { href: number, text: string, image?: string }) => {
    ...
    return <Button className={className} onClick={() => {
      navigate(`/company/chat/${href}`, { replace: true });
    }}>
    ...
    <div className='classDiv gap-3 flex min-h-[80vh]'>
      <div className='w-3/12 bg-slate-200 p-2 rounded-md shadow-md flex flex-col gap-2'>
        ...
        {data?.data.map((i, index) => (
          <NavLink href={i.id} text={i.expert.name} key={index} image={i.expert.image} />
        ))}
