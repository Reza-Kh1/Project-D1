...
export default function ChatBox() {
    ...
    const MessageComponent = ({ text, type, image, id }: MessageComponentType) => {
        return (
            <div>
                <div className={`${type === "COMPANY" ? "bg-gray-500 border-message-right ml-20 mr-10" : "bg-blue-400 border-message-left mx-10"} text-white text-sm leading-8 shadow-md p-2 relative`}>
                    {text}
                    {type === 'COMPANY' ?
                        <button title='trash button' onClick={() => {
                            setIdDelete(id)
                            setOpen(true)
                        }} className={`${type === "COMPANY" ? "-left-9" : "-right-8"} absolute top-1/2 p-2 bg-black/40  rounded-full shadow-md transform -translate-y-1/2`} type='button'>
                            <FaTrash className='text-white' />
                        </button>
                        : null}
                </div>
                <ImageCustom alt="use test" className='w-10 h-10 shadow-md rounded-full' figureClass={`relative w-full flex ${type === "EXPERT" ? "justify-end" : ""}`}  src={type === "EXPERT" ? image ? image : "/user.jpg" : "/company.png"} width={40} height={40} />
            </div>
        )
    }
    useEffect(() => {
